...
class RecyclerAdapterSubActive(private val context: Context, listenerActiveSub: SubActiveFragment) : RecyclerView.Adapter<RecyclerAdapterSubActive.ViewHolder>() {
    ...
    private var dataListSub: List<DataModelActiveSub> = emptyList<DataModelActiveSub>()
    var onItemClicked : ((DataModelActiveSub) -> Unit)? = null
    var onItemClickedPause : ((DataModelActiveSub) -> Unit)? = null
    var onItemClickedCancel : ((DataModelActiveSub) -> Unit)? = null


    internal fun setDataList(dataList: List<DataModelActiveSub>) {
