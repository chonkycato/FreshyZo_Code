...
class ApiCall {
    ...
    fun auth(email: String, password: String, context:Application, callback: LoginCallback){
//        println(email)
//        println(password)
...
//                println(response::class.simpleName)
    ...
    fun submitDelivery(customerId: String, agentId: String,arr: JSONArray, extraOrder: Boolean, context: Context, callback: SubmitDeliveryCallback){
        ...
        println(jsonObject)
        val urlAdd: String = if(extraOrder) "submit_extra_order" else "submit_today_order"
        val queue = Volley.newRequestQueue(context)
        val request = JsonObjectRequest(
